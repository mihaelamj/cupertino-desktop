...
- **`LocalSubprocessBackend` over a remote `Transport.Channel` (future)**: same client, but network reachability, auth, and latency become the failure modes; same observable state machine.
