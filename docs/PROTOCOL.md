...
- The subprocess adapter reuses cupertino's cross-platform `MCPCore` types on the wire (via `MCPClientKit`); the embedded adapter reuses cupertino's typed read services and result models. Both map into the `AppModels` above and expose nothing else.
