...
Canonical dependency-injection principles for XCTemplateDSL and the wider XCTemplateDSL codebase: no singletons, constructor injection, protocol seams.
...
If and when XCTemplateDSL grows into multiple SPM packages:
...
If and when XCTemplateDSL is split into multiple packages, each package should lift out of the repo without modification:
...
If XCTemplateDSL ships multiple binaries, the **executable target name** stays normal (no `Impl` suffix). The binary's external identity (filename on disk, `which` output) stays clean.
